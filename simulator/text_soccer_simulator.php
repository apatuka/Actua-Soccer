...
const ALLOWED_POSITIONS = ['GK','DF','MF','FW'];
const PITCH_X = 100.0;
const PITCH_Y = 64.0;
...
function normalizePosition(string $pos): string {
    ...
    return in_array($u, ALLOWED_POSITIONS, true) ? $u : 'MF';
}
...
function normalizeTeam(array $team, string $fallbackName): array {
    ...
    if (!is_array($players) || count($players) === 0) {
        $defaults = ['GK','DF','DF','DF','DF','MF','MF','MF','FW','FW','FW'];
        for ($i = 1; $i <= 11; $i++) {
            $p = ['name' => "{$name} Player {$i}", 'position' => $defaults[$i-1]];
    ...
    foreach ($players as $i => $p) {
        $n = [
            'name' => (string)($p['name'] ?? "{$name} Player " . ($i+1)),
            'position' => normalizePosition((string)($p['position'] ?? ($i === 0 ? 'GK' : 'MF'))),
        ];
        foreach (SOURCE_PLAYER_ATTRS as $attr) {
    ...
    while (count($base['players']) < 11) {
        ...
        $n = ['name' => "{$name} Player {$i}", 'position' => 'MF'];
        foreach (SOURCE_PLAYER_ATTRS as $attr) {
            $n[$attr] = $base[$attr];
        }
        $base['players'][] = $n;
...
function initPositions(array $team, bool $home): array {
    ...
    $countByPos = ['GK' => 0, 'DF' => 0, 'MF' => 0, 'FW' => 0];
    foreach ($team['players'] as $i => $p) {
        $position = normalizePosition((string)($p['position'] ?? ($i === 0 ? 'GK' : 'MF')));
        if ($i === 0 && $position !== 'GK') {
            $position = 'GK';
        }
        $slot = $countByPos[$position] % count($lanes);
        $countByPos[$position]++;
        $sx = $baseX + ($lineByPos[$position] * $dir);
        $sy = $lanes[$slot];
        $out[] = [
            'name' => $p['name'],
            'attrs' => $p,
...
function cpuDecision(array &$state, int $minute): void {
    ...
    $shootRoleBonus = $holder['role'] === 'FW' ? 0.10 : ($holder['role'] === 'MF' ? 0.03 : -0.07);
    $shootChance = 0.08 + (($holder['attrs']['accuracy'] + $holder['attrs']['power']) / 260) - ($distanceToGoal / 140) + $shootRoleBonus;
    ...
    if ($targetIdx !== null && $inCrossArea) {
        foreach ($state[$teamKey]['players'] as $i => $candidate) {
            if ($candidate['role'] === 'FW' && $i !== $holderIdx) {
                $targetIdx = $i;
                break;
            }
        }
